...
## 6. 성능 및 정확도 벤치마크 (Benchmark Results)

`whereismykey`의 해시 매칭 엔진과 비동기 오케스트레이션 파이프라인은 고속 정규식 토큰 분해 및 SHA-256 대조 알고리즘을 통해 **오탐율 0%**와 **초당 70만 건 이상의 문서 처리 속도**를 제공합니다.
...
#### 1) 정확도 및 오탐 방지 검증 (Accuracy & Precision)
| 검증 항목 | 설명 | 측정 결과 |
|---|---|:---:|
| **정답 키 탐지율 (Recall)** | 실제 노출 키가 포함된 문서의 확정(`confirmed`) 탐지 성공률 | **100.0%** |
| **해시 충돌 방어율 (FP = 0)** | 동일 prefix/postfix를 가진 유사 키를 `confirmed`로 오탐하지 않고 `pattern_only`로 격리 | **100.0%** |
| **토큰 경계 검사 (Boundary)** | 키 앞뒤에 영숫자가 붙어 식별자 일부로 쓰인 경우 오탐 방지 | **100.0%** |
| **일반 코드 노이즈 차단율** | 무관한 소스코드/설정파일 수천 라인 대상 오탐 발생 여부 | **100.0% (오탐 0건)** |
| **평문 키 마스킹 보장** | 결과 스니펫 및 리포트에서 평문이 `prefix…postfix`로 가려지는지 검증 | **100.0%** |

#### 2) 처리 속도 및 처리량 (Speed & Throughput)
| 측정 지표 | 성능 수치 |
|---|---|
| **1,000개 문서 배치 스캔 소요 시간** | **1.28 ms** |
| **문서당 평균 처리 시간** | **0.001 ms / doc** |
| **초당 문서 스캔량** | **~780,000 docs / s** |
| **배치 처리 Throughput** | **126.93 MB / s** |
| **5MB 대용량 텍스트 단일 스캔 시간** | **37.74 ms** (132.47 MB/s) |

#### 3) 비동기 E2E 오케스트레이션 지연시간 (End-to-End Pipeline)
| 항목 | 설명 | 측정치 |
|---|---|:---:|
| **10개 소스 병렬 파이프라인** | 10개 검색 소스 병렬 수집 + 매칭 + 진행률 갱신 + ScanResult 빌드 | **56.37 ms** |
| **최종 판정 (Verdict)** | 노출 키 확정 판정 정상 도출 | **`EXPOSED`** |
